...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BTCRYX")
    updateDisplayUnit();
}
...
{
    QString ret = QString("bitcrystalrare:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTCRYX unit input in lnReqAmount, we generate the URI with BTCRYX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BTCRYX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
